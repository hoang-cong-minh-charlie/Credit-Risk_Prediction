# ==============================================================================
# QUY TRÌNH NGHIÊN CỨU: 5-FOLD CROSS-VALIDATION STACKING (SEQUENTIAL EXECUTION)
# ĐIỀU CHỈNH: K=20 (GAM), TREE_DEPTH=8 (XGBoost), N_RUNS=10
# ==============================================================================
...
# ==========================
n_runs <- 10 # 👈 ĐIỀU CHỈNH: N_RUNS = 10
K_FOLDS <- 5 
cat("⏳ Running sequential 5-Fold Stacking for", n_runs, "runs.\n")

# Định nghĩa công thức GAM (ĐIỀU CHỈNH: K=20)
...
for (i in 1:n_runs) {
  ...
  # Chuẩn bị OOF Prediction vectors (có cùng số hàng với data_train_full)
  ...
  for (k in 1:K_FOLDS) {
    ...
    # 1. XGBOOST K-FOLD Training (ĐIỀU CHỈNH: TREE_DEPTH=8)
    ...
    xgb_spec <- parsnip::boost_tree(trees = 1000, tree_depth = 8, learn_rate = 0.1) %>% # 👈 TREE_DEPTH = 8
    ...
    # 3. OOF PREDICTIONS (Tạo Meta-Features)
  ...
  # --------------------------------------------------
  # C. BASE MODEL FINAL TRAINING (TRÊN TOÀN BỘ data_train_full)
  # --------------------------------------------------
  ...
  cvfit <- cv.glmnet(
    ...
    penalty.factor = c(1, 0.01), # XGBoost=1, GAM=0.01 (Ưu tiên GAM)
...
saveWorkbook(wb, "results_runs_optimized_final_k20_d8_n10.xlsx", overwrite = TRUE)
cat("✅ Exported all results to results_runs_optimized_final_k20_d8_n10.xlsx\n")
