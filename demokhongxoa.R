...
run_experiment <- function(seed_val = 123) {
  ...
  # ---- Recipe for XGBoost (dummy + SMOTE) ----
  ...
  # ---- Predictions ----
  data_test_prep <- bake(prep(xgb_rec), new_data = data_test)
  
  xgb_prob <- predict(xgb_final, new_data = data_test_prep, type = "prob")$.pred_1
  ...
  xgb_eval <- tibble(lst = data_test$lst,
                     .pred_class = predict(xgb_final, new_data = data_test_prep, type = "class")$.pred_class,
                     .pred_1 = xgb_prob)
...
cat("✅ Saved to results_30_runs.xlsx\n")
